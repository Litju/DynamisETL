...
def test_partial_acquisition_preserves_earlier_provenance(
    ...
    # A manifest generated by an older, smaller registry must be extended when
    # the pinned registry learns about another asset without losing A's history.
    existing = read_bronze_manifest(tmp_settings, dataset_id="syn-provider", version="v1")
    write_bronze_manifest(tmp_settings, existing.model_copy(update={"files": existing.files[:1]}))
